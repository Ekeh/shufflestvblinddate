...
<div style="color:yellow; padding: 10px;">Only Content in "I Want Your X" are counted here</div>
<div style="color:yellow; padding: 10px;">Top 10 Female and Top 10 Male</div>
<br>

<?php
  $tot=mysqli_query($db,"SELECT  SUM(views) AS sumtviews,vplace FROM tbl_vip_videos WHERE xrating='9' ");   
...
WHERE views>0 and xrating='9' and gender='female'
GROUP BY actor_id 
...
$query=mysqli_query($db,"SELECT  SUM(views) AS sumviews,actor_id,vplace,fname,lname,phone,gender  FROM tbl_vip_videos 
INNER JOIN tbl_users ON  tbl_vip_videos.actor_id=tbl_users.userid  
WHERE views>0 and xrating='9' and gender='male'
GROUP BY actor_id 
ORDER BY SUM(views) DESC 
limit 10");   
      while($row=mysqli_fetch_array($query))
