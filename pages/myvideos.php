...
$msg= "<div class='alert-success alert'> Successfully Deleted </div>";
  $chkmsg="success";
}else{
...
if(isset($_POST['deleteusernow'])){
   $postid=$_POST['postid'];
  $msg= "<div class='alert-danger alert'> If you continue, the video will be permanently deleted<br>
 <a href='".SITE_URL."/index.php?p=myvideos&postid=".$postid."&delete=1' class='button warning'> CONTINUE</a></div>";
...
if ($_FILES["video"]["error"] > 0)
    {
        $msg.= "<div class='alert-danger alert'>Invalid Video</div>";
        $error++;
    }
    else
    {
...
if (move_uploaded_file($tmp,$filepath.$uploadedVideoPath)) { $msg.=  "<div class='alert-danger alert'>Video Uploaded</div>";  $chkmsg="success"; }else{$msg.="<div class='alert-danger alert'>Could not upload video</div>";  $chkmsg="error"; $error++; }
...
///// ends here 
  ...
  if($coverimage!='' && $name!=''){
      $sql3 = mysqli_query(
                    $db,"UPDATE tbl_vip_videos set  vid_title='$vid_title',vid_desc='$vid_desc',video='$uploadedVideoPath', coverimage='$newname',video_amount='$vid_amount',xrating='$vrating',vplace='$vplace' where video_id='$postid'");
  }else
  if($coverimage!='' && $name==''){
      $sql3 = mysqli_query(
                    $db,"UPDATE tbl_vip_videos set  vid_title='$vid_title',vid_desc='$vid_desc', coverimage='$newname',video_amount='$vid_amount',xrating='$vrating',vplace='$vplace' where video_id='$postid'");
  }else
  if($coverimage=='' && $name!=''){
      $sql3 = mysqli_query(
                    $db,"UPDATE tbl_vip_videos set  vid_title='$vid_title',vid_desc='$vid_desc',video='$uploadedVideoPath',video_amount='$vid_amount',xrating='$vrating',vplace='$vplace' where video_id='$postid'");
  }else
  if($coverimage=='' && $name==''){
      $sql3 = mysqli_query(
                    $db,"UPDATE tbl_vip_videos set  vid_title='$vid_title',vid_desc='$vid_desc',video_amount='$vid_amount',xrating='$vrating',vplace='$vplace' where video_id='$postid'");
  }
...
<form method="POST" action="?p=myvideos">
 ...
 <button type="submit" class="btn btn-primary"  name="edituser"><span class="fa fa-edit"></span> Edit</button> <button type="submit" class="btn btn-danger"  name="deleteusernow"><span class="fa fa-delete"></span> Delete</button>
 <div><span class=''>Pending Approval</span></div>
<?php }else{ ?>
