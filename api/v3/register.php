...
if($numCount > 0) {
        $result =  ["success" => false, "message" => "Your device is already link to an account. You cannot use it to register another account.", "data"=> null];
        mysqli_close($db);
        echo json_encode($result);
        exit();
}
...
if($nums2!='0'){
    ...
    $sql3 = mysqli_query(
        $db,"INSERT into tbl_users set email='$stremail', password='$password',fname='$strfname',lname='$strlname', phone='$strphone',useripaddress='$useripaddress', shuffles_app='1', activation_status = '1', device_imei = '$deviceImei'"
    );
